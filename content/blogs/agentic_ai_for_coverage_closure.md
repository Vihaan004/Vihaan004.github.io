...
We're building an agentic framework that does this autonomously. An AI system that reads a design specification, generates SystemVerilog testbenches, runs simulations, parses coverage reports, and iterates, all without human intervention. The goal: full code and functional coverage closure, driven by LLM-powered agents orchestrated through LangGraph.
...
Prior work in LLM-assisted verification has explored assertion generation, testbench scaffolding, and stimulus creation. ChiRAAG uses ChatGPT for iterative assertion refinement. LLM4DV demonstrated LLM-driven stimulus generation from functional coverpoints. VerilogReader approaches coverage-centric test generation by exposing design code to the LLM. But most of these efforts target narrow slices of the verification problem. Automated coverage closure from specifications, the way an engineer actually works, remains largely unaddressed.

## The Foundation: A ReAct Agent That Verifies

Our framework starts with a ReAct (Reasoning + Acting) agent built on LangGraph. The agent has access to a set of modular tools: file operations for reading specs and writing testbenches, simulation tools that compile and run designs through QuestaSim or Verilator, and analysis tools that parse coverage databases and identify exactly which lines remain uncovered.
